#!/usr/bin/env Rscript
# brier_plot_box.R - wrap BRIER::plot.box for bootstrap performance
# comparison across target-only / external-only / integrated BRIER models.
#
# Returns PNG + CSV of the underlying bootstrap distributions.
#
# Inputs (JSON):
#   selection_id, data_path, newx_expr, newy_expr, criteria
#   covar_expr (optional), adjust_covar (optional), standardize (bool)
#   bootstrap_n (default 100), seed (optional)
#   width, height, dpi
#
# Outputs:
#   {status, plot_id, plot_png_path, plot_csv_path, criteria, n_bootstrap}
...
.generate_plot_id <- function(prefix = "plot_box") {
...
result <- tryCatch({
  ...
  plot_id <- .generate_plot_id("plot_box")
  ...
  if (!is.null(res$plot) && requireNamespace("ggplot2", quietly = TRUE)) {
    ggplot2::ggsave(png_path, plot = res$plot,
                    width = width/dpi, height = height/dpi,
                    dpi = dpi, units = "in")
  } else {
    stop("plot.box did not return a ggplot; cannot render", call. = FALSE)
  }

  # The bootstrap matrix has rows per replicate, columns per model variant
  # Save as long-format CSV for plotting
  boot_mat <- res$bootstrap.mat
  if (!is.null(boot_mat)) {
    boot_df <- as.data.frame(boot_mat)
    boot_df$bootstrap_replicate <- seq_len(nrow(boot_df))
  ...
  list(
    ...
    plot_csv_path = if (!is.null(boot_mat)) csv_path else NA_character_,
    criteria = inp$criteria,
