...
_CORE_TOOLS = {
    ...
    "brier_evaluate", "score_external_models",
...
def _decision_summary(tool_results):
    ...
    transfer metric (brier_evaluate) and the external-only metric (score_external_models) report
    ...
    for tr in (tool_results or []):
        ...
        if tr.get("tool") == "brier_evaluate":
            transfer = float(val)
        elif tr.get("tool") == "score_external_models":
            external = float(val)
